...
export const ChatInput = ({
  ...
  const handleAgentModeClick = () => {
    ...
    // All users have ultra - agent mode always available
    setChatMode("agent");
  };

  const handleAgentLongModeClick = () => {
    // Local/dev builds don't have Trigger.dev configured for Agent-Long,
    // so fall back to regular Agent mode instead of calling /api/agent-long.
    if (temporaryChatsEnabled) {
    ...
    toast.info("Agent-Long mode isn't available locally", {
      description: "Using Agent mode instead.",
    });
    setChatMode("agent");
  };
    ...
    const handlePaste = async (e: ClipboardEvent) => {
      ...
      if (textareaRef.current === document.activeElement) {
        ...
        if (clipboardData) {
          ...
          if (pastedText) {
            // Check token limit for the pasted text - ultra users have high limit
  ...
  return (
    <div className={`relative px-4 ${isCentered ? "" : "pb-3"}`}>
      <div className="mx-auto w-full max-w-full sm:max-w-[768px] sm:min-w-[390px] flex flex-col flex-1">
        ...
        {isNewChat && (
          <div className="order-1 sm:order-2 flex items-center gap-2 px-1 pb-2 sm:pt-2 sm:pb-0 min-h-9">
            <ModelSelector />
