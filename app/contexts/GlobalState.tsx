...
export const GlobalStateProvider: React.FC<GlobalStateProviderProps> = ({
  ...
  // Start with a fixed default on both server and client to avoid hydration mismatches.
  ...
  // Hydrate chat mode preference from localStorage after mount (client-only)
  useEffect(() => {
    const saved = readChatMode();
    if (isChatMode(saved)) {
      // Map agent-long -> agent for local/dev where Trigger.dev isn't configured
      setChatMode(saved === "agent-long" ? "agent" : saved);
    }
  }, []);
  ...
  // Persist model preference to localStorage on change
  useEffect(() => {
    if (selectedModel) {
  ...
  // Derive subscription tier - always return ultra for all users
  useEffect(() => {
    setSubscription("ultra");
  }, []);

  // Always keep subscription as ultra
  useEffect(() => {
