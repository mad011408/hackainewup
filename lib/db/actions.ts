...
export async function getChatById({ id }: { id: string }) {
  // If no Convex URL is configured, return null (new chat)
  if (!process.env.NEXT_PUBLIC_CONVEX_URL || process.env.NEXT_PUBLIC_CONVEX_URL.includes("placeholder")) {
    return null;
  }
...
export async function saveChat({
  ...
  // If no Convex URL is configured, skip saving (dev mode without database)
  if (!process.env.NEXT_PUBLIC_CONVEX_URL || process.env.NEXT_PUBLIC_CONVEX_URL.includes("placeholder")) {
    return id;
  }
...
export async function updateChat({
  ...
  // If no Convex URL is configured, skip updating (dev mode without database)
  if (!process.env.NEXT_PUBLIC_CONVEX_URL || process.env.NEXT_PUBLIC_CONVEX_URL.includes("placeholder")) {
    return chatId;
  }
...
export async function getUserCustomization({ userId }: { userId: string }) {
  // If no Convex URL is configured, skip querying (dev mode without database)
  if (!process.env.NEXT_PUBLIC_CONVEX_URL || process.env.NEXT_PUBLIC_CONVEX_URL.includes("placeholder")) {
    return null;
  }
...
export async function startStream({
  ...
  // If no Convex URL is configured, skip (dev mode without database)
  if (!process.env.NEXT_PUBLIC_CONVEX_URL || process.env.NEXT_PUBLIC_CONVEX_URL.includes("placeholder")) {
    return;
  }
...
export async function createNote({
  ...
  // If no Convex URL is configured, skip creating (dev mode without database)
  if (!process.env.NEXT_PUBLIC_CONVEX_URL || process.env.NEXT_PUBLIC_CONVEX_URL.includes("placeholder")) {
