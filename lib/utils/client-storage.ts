...
export const readSystemPrompt = (): string | null => {
  if (!isBrowser()) return null;
  try {
...
export const writeSystemPrompt = (prompt: string): void => {
  if (!isBrowser()) return;
  try {
    window.localStorage.setItem(SYSTEM_PROMPT_STORAGE_KEY, prompt);
  } catch {
    // ignore
  }
};
